...
type StatusType = 'compliant' | 'partial' | 'gap' | 'na';
...
function drawRect(page: PDFPage, x: number, y: number, w: number, h: number, color: ReturnType<typeof rgb>, border?: ReturnType<typeof rgb>) {
  page.drawRectangle({ x, y: y - h, width: w, height: h, color, borderColor: border, borderWidth: border ? 0.5 : 0 });
}
...
function sectionHead(ctx: Ctx, label: string, subtitle?: string): Ctx {
  ...
  drawRect(page, M, y, W - M * 2, bH, C.lightGreen, C.green);
  ctx.page.drawRectangle({ x: M, y: y - bH, width: 4, height: bH, color: C.darkGreen });
...
function kpiRow(ctx: Ctx, items: Array<{ label: string; value: string; unit?: string; color?: ReturnType<typeof rgb>; bg?: ReturnType<typeof rgb> }>): Ctx {
  ...
  items.forEach((item, i) => {
    ...
    drawRect(page, cx, y, cW, cH, item.bg || C.nearWhite, C.divider);
    page.drawRectangle({ x: cx, y: y - cH, width: 3, height: cH, color: col });
...
function statusBadge(page: PDFPage, status: StatusType, x: number, midY: number, font: PDFFont) {
  const bW = 72, bH = 16;
  // drawRect takes y as TOP — center badge on midY
  drawRect(page, x, midY + bH / 2, bW, bH, STATUS_BG[status], STATUS_COLOR[status]);
...
function complianceTable(ctx: Ctx, rows: ComplianceRow[]): Ctx {
  ...
  ctx = need(ctx, rH * 2);
  drawRect(ctx.page, M, ctx.y, totalW, rH, C.blue);
  let hx = M;
  headers.forEach((h, i) => {
...
function actionTable(ctx: Ctx, items: ActionItem[]): Ctx {
  ...
  ctx = need(ctx, rH * 2);
  drawRect(ctx.page, M, ctx.y, totalW, rH, C.darkGreen);
  let hx = M;
  headers.forEach((h, i) => {
  ...
  items.forEach((item, i) => {
    ...
    drawRect(ctx.page, rx + 4, ctx.y - 8, cols[4] - 12, 14, rgb(ic.red * 0.15 + 0.85, ic.green * 0.15 + 0.85, ic.blue * 0.15 + 0.85), ic);
...
function complianceScoreBar(ctx: Ctx, score: number, label: string): Ctx {
  ctx = need(ctx, 32);
  const { page, y, M, W } = ctx;
  const barW = W - M * 2;
  const barH = 22;

  drawRect(page, M, y, barW, barH, C.lightGray, C.divider);
  // Fill
  const fillW = Math.round((score / 100) * barW);
  const fillCol = score >= 75 ? C.green : score >= 50 ? C.amber : C.red;
  drawRect(page, M, y, fillW, barH, fillCol);
  // Label
  drawText(page, `${label}: ${score}%`, M + 8, y - 14, 9, ctx.font, score >= 75 ? C.white : C.darkGray);
  drawText(page, score >= 75 ? 'Добро ниво на съответствие' : score >= 50 ? 'Частично съответствие' : 'Необходими са действия',
    M + barW - 180, y - 14, 8, ctx.font, score >= 75 ? C.white : C.darkGray);
...
export async function generateComplianceReport(data: ComplianceReportData): Promise<Buffer> {
  ...
  const hasScope3 = data.scope3Available && data.scope3Total > 0;
  const hasTargets = data.targets && data.targets.length > 0;
  const hasGoodData = (data.scope1Total + data.scope2Total) > 0;

  // Compute overall compliance score
  let score = 0;
  if (hasGoodData) score += 30;          // Has S1+S2 data
  if (hasScope3) score += 20;             // Has S3 data
  if (hasTargets) score += 20;            // Has reduction targets
  if (data.company.registration_number) score += 10; // Company registered
  if (data.reportingYear >= new Date().getFullYear() - 1) score += 10; // Recent data
  score += 10; // Base for using a reporting platform

  // ── Regulatory rows ──────────────────────────────────
  const regulations: ComplianceRow[] = [
    {
      ...
      note: hasGoodData ? 'Scope 1+2 измерени и документирани' : 'Необходимо е събиране на данни',
    },
    {
      regulation: 'Наредба 7/1999 (Качество на атм. въздух)',
      requirement: 'Мониторинг на стационарни горивни инсталации',
      status: data.scope1Total > 0 ? 'compliant' : 'na',
      note: data.scope1Total > 0 ? 'Данните за горива са включени в отчета' : 'Не е приложимо — няма стационарни инсталации',
    },
    {
      regulation: 'EU ETS (Схема за търговия с емисии)',
      requirement: 'Задължително само за инсталации >20 MW или >25 kt CO2e/год.',
      status: grandTotal < 25 ? 'na' : 'partial',
      note: grandTotal < 25 ? `${grandTotal.toFixed(1)} tCO2e — под прага за задължително участие` : 'Препоръчва се консултация с компетентен орган',
    },
    {
      regulation: 'CSRD (Директива 2022/2464/ЕС)',
      requirement: 'Задължителна ESRS E1 климатична отчетност (от 2025-2026)',
      status: hasGoodData && hasScope3 ? 'compliant' : hasGoodData ? 'partial' : 'gap',
      note: hasGoodData && hasScope3 ? 'Scope 1+2+3 отчетени по ESRS E1' : hasGoodData ? 'Scope 3 все още не е пълно' : 'Необходима е пълна GHG инвентаризация',
    },
    {
      regulation: 'EU Taxonomy (Регламент 2020/852)',
      requirement: 'Оценка на климатичното съответствие на дейностите',
      status: 'partial',
      note: 'Препоръчва се скрининг на дейностите по Приложение I и II',
    },
    {
      regulation: 'GHG Protocol Corporate Standard',
      requirement: 'Scope 1, 2 и материален Scope 3 с одитен след',
      status: hasGoodData && hasScope3 ? 'compliant' : hasGoodData ? 'partial' : 'gap',
      note: hasGoodData && hasScope3 ? 'Всички обхвати измерени и документирани' : 'Scope 3 данните са непълни',
    },
    {
      regulation: 'SBTi (Научно-базирани цели)',
      requirement: 'Поставени цели, одобрени от SBTi инициативата',
      status: hasTargets ? 'partial' : 'gap',
      note: hasTargets ? 'Цели са поставени, но все още не са верифицирани от SBTi' : 'Не са поставени количествени цели за намаляване',
    },
    {
      regulation: 'ISO 14064-1:2018',
      requirement: 'Документирана методология и вътрешен одит',
      status: hasGoodData ? 'partial' : 'gap',
      note: hasGoodData ? 'Методологията е документирана чрез ZED Platform. Препоръчва се вътрешен одит.' : 'Необходима е документирана методология',
    },
  ];

  // ── Action plan ─────────────────────────────────────
  const actions: ActionItem[] = [];
  if (!hasScope3) actions.push({ priority: 1, action: 'Импортирайте финансови транзакции за изчисляване на Scope 3 емисии чрез ZED Platform', regulation: 'CSRD / GHG Protocol', timeline: '1-2 месеца', impact: 'висок' });
  if (!hasTargets) actions.push({ priority: 2, action: 'Поставете количествена цел за намаляване на емисиите (препоръка: -30% до 2030 г. спрямо базова година)', regulation: 'SBTi / CSRD E1-4', timeline: '1-3 месеца', impact: 'висок' });
  actions.push({ priority: actions.length + 1, action: 'Преминете към 100% ВЕИ тарифа за електроенергия — намалява Scope 2 до нула', regulation: 'CSRD E1-6 §45', timeline: '1-6 месеца', impact: 'висок' });
  actions.push({ priority: actions.length + 1, action: 'Изпратете климатичен въпросник до Топ 10 доставчика по разходи за подобряване на Scope 3 качеството', regulation: 'GHG Protocol / SBTi', timeline: '3-6 месеца', impact: 'среден' });
  actions.push({ priority: actions.length + 1, action: 'Извършете EU Taxonomy скрининг — оценете съответствието на основните дейности с климатичните цели', regulation: 'EU Taxonomy', timeline: '3-6 месеца', impact: 'среден' });
  actions.push({ priority: actions.length + 1, action: 'Подгответе се за CSRD верификация — потърсете ограничена (limited assurance) верификация по ISAE 3410', regulation: 'CSRD / ESRS E1', timeline: '6-12 месеца', impact: 'среден' });
  if (grandTotal >= 25) actions.push({ priority: actions.length + 1, action: 'Консултирайте се с РИОСВ относно EU ETS задължения — проверете дали инсталациите надхвърлят прага', regulation: 'EU ETS', timeline: 'Незабавно', impact: 'висок' });
  ...
  drawRect(firstPage, M, badgeTop, 120, 14, C.green);
  ...
  drawText(firstPage, `Регулаторна рамка — ЗООС, CSRD, EU ETS, SBTi  |  ${data.reportingYear} г.`, M + 4, H - HDR + 55, 9.5, font, rgb(0.72, 0.84, 1.0));

  // Company info box
  const cbTop = H - HDR - 10;
  drawRect(firstPage, M, cbTop, W - M * 2, 68, C.white, C.divider);
  firstPage.drawRectangle({ x: M, y: cbTop - 68, width: 4, height: 68, color: C.green });
  ...
  drawText(firstPage, `Отчетна година: ${data.reportingYear}   |   Генериран: ${today}   |   ${data.generatedBy || 'ZED Platform'}`, M + 12, cbTop - 52, 8, font, C.gray);

  // Score banner
  const scoreBanTop = cbTop - 68 - 12;
  const fillCol = score >= 75 ? C.green : score >= 50 ? C.amber : C.red;
  drawRect(firstPage, M, scoreBanTop, W - M * 2, 60, C.nearWhite, C.divider);
  firstPage.drawRectangle({ x: M, y: scoreBanTop - 60, width: 4, height: 60, color: fillCol });
  drawText(firstPage, 'ИНДЕКС НА СЪОТВЕТСТВИЕ', M + 12, scoreBanTop - 12, 8, font, C.gray);
  const scoreStr = `${score}%`;
  drawText(firstPage, scoreStr, M + 12, scoreBanTop - 44, 30, font, fillCol);
  const statusLabel = score >= 75 ? 'Добро ниво — продължете подобренията' : score >= 50 ? 'Частично съответствие — нужни са действия' : 'Ниско ниво — необходими са спешни мерки';
  drawText(firstPage, statusLabel, M + 12 + font.widthOfTextAtSize(scoreStr, 30) + 10, scoreBanTop - 36, 10, font, fillCol, W - M * 2 - 100);

  // Progress bar for score
  const pbTop = scoreBanTop - 50;
  const pbW = W - M * 2 - 16;
  drawRect(firstPage, M + 8, pbTop, pbW, 8, C.lightGray);
  drawRect(firstPage, M + 8, pbTop, Math.round((score / 100) * pbW), 8, fillCol);

  // Emissions summary boxes (3 scopes)
  const boxAreaTop = scoreBanTop - 60 - 12;
  const boxH2 = 96;
  const boxGap = 7;
  ...
  [
    ...
    drawRect(firstPage, cx, boxAreaTop, boxW2, boxH2, bg, color);
    // bottom accent stripe — 6 px above box bottom to leave text breathing room
    firstPage.drawRectangle({ x: cx, y: boxAreaTop - boxH2, width: boxW2, height: 5, color });
  ...
  drawRect(firstPage, M, legTop, W - M * 2, 34, C.nearWhite, C.divider);
  ...
  drawText(firstPage, `Отчетна година: ${data.reportingYear}  |  Метод: GHG Protocol  |  Верификация: Вътрешна`, M + 10, legTop - 26, 8, font, C.gray);
  ...
  // Legend
  ctx = need(ctx, 24);
  const legendItems: [StatusType, string][] = [['compliant', 'Изпълнено'], ['partial', 'Частично'], ['gap', 'Пропуск'], ['na', 'Не се прилага']];
  legendItems.forEach(([s, l], li) => {
    const lx = ctx.M + li * 118;
    drawRect(ctx.page, lx, ctx.y, 10, 10, STATUS_BG[s], STATUS_COLOR[s]);
  ...
  // Score breakdown
  ctx = sectionHead(ctx, 'Обобщен индекс на съответствие по категория');
  const compliant = regulations.filter(r => r.status === 'compliant').length;
  const partial = regulations.filter(r => r.status === 'partial').length;
  const gap = regulations.filter(r => r.status === 'gap').length;
  const total2 = regulations.filter(r => r.status !== 'na').length;

  ctx = complianceScoreBar(ctx, Math.round((compliant / total2) * 100), `Пълно съответствие (${compliant} от ${total2} регулации)`);
  ctx = complianceScoreBar(ctx, Math.round(((compliant + partial) / total2) * 100), `Пълно + Частично (${compliant + partial} от ${total2})`);
  ctx = spacer(ctx, 4);
  ctx = para(ctx, `От ${regulations.length} проверени регулации: ${compliant} изпълнени, ${partial} частично, ${gap} с пропуски. Препоръчано е да се адресират пропуските в приоритетен ред.`);
  ...
  ctx = para(ctx, `Съгласно Директива CSRD (2022/2464/ЕС), компании с над 250 служители или над 40 млн. EUR оборот ще бъдат задължени да отчитат по ESRS E1 от 2025/2026 г. По-малките компании могат да бъдат включени в обхвата чрез веригата за доставки на по-големи партньори.`);
  ctx = spacer(ctx, 4);
  ...
  ctx = need(ctx, 52);
  ctx = spacer(ctx, 8);
  drawRect(ctx.page, ctx.M, ctx.y, ctx.W - ctx.M * 2, 44, C.nearWhite, C.divider);
  drawText(ctx.page, 'Правно уведомление', ctx.M + 10, ctx.y - 12, 8.5, ctx.font, C.darkGray);
  const disclaimer = 'Настоящият отчет е с информационна цел и не представлява правен или регулаторен съвет. Точността зависи от предоставените данни. За задължителна регулаторна отчетност се препоръчва консултация с квалифициран EHS/ESG специалист.';
